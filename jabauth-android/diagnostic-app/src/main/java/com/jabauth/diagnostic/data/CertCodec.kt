...
/**
 * Pure DER↔hex codec for persisting X.509 trust anchors as DataStore strings. No Android or DataStore
 * dependency, so it is unit-tested on the plain JVM. Hex (rather than `android.util.Base64`) keeps it both
 * pure-JVM and minSdk-24 safe (`java.util.Base64` is API 26+); the size overhead is negligible for a
 * handful of anchors.
 */
object CertCodec {
