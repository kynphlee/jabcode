...
public class DecodingBenchmark extends BenchmarkBase {
    ...
    // Full WS-5 era coverage: all 8 Nc values (2/4/8/16/32/64/128/256).
    // The previous parametrization (4..128) missed Nc=0 (monochrome,
    // newly supported per panama-poc commit "WS-6.5 accept color_number=2")
    // and Nc=7 (256-color, the most extreme palette).
    //
    // Cross-reference: today's 8-Nc Android discriminator scan revealed
    // the H_partI_unifies cluster {Nc=0, Nc=2, Nc=7} fingerprints
    // identically — extending this benchmark to cover those Nc values
    // surfaces the JVM-side timing fingerprint and lets the JVM-side data
    // be cross-corroborated against the Android-side numbers.
