...
public class EncodingBenchmark extends BenchmarkBase {
     ...
     *
     * Nc=0 (2-color / monochrome): newly supported on panama-poc commit
     * "WS-6.5 accept color_number=2" — included for parity with the
     * Android benchmarks and full Nc=0..7 coverage.
     *
     * Nc=7 (256-color): historically excluded due to an encoder malloc
     * issue in the panama-poc era; the WS-5 work may have fixed this.
     * Re-include after running this benchmark and confirming no crash;
     * if it still fails, file an `H_nc7_encoder_malloc` register entry
     * and exclude again.
     */
